...
import { generateSSOToken, storeSSOToken, initTokenRefresh } from "../services/ssoService";
...
const LoginPage = () => {
  ...
  const welcomeText = "Welcome To Passary Refractories";
  ...
  useEffect(() => {
    if (isLoggedIn && userData) {
      ...
      // ========== STORE PASSWORD FOR AUTO-FILL ==========
      // Store password in encoded format for auto-fill in external apps
      if (formData.password) {
        const encodedPassword = btoa(formData.password); // Simple encoding
        localStorage.setItem("user-password", encodedPassword);
      }
      // ========== END PASSWORD STORAGE ==========

      // Store accessible app IDs
      if (userData.accessibleAppIds) {
        localStorage.setItem("accessibleAppIds", JSON.stringify(userData.accessibleAppIds));
      }

      // ========== ENHANCED SSO INTEGRATION ==========
      // Generate SSO token using the service
      ...
      if (ssoToken) {
        // Store SSO token in all locations
        storeSSOToken(ssoToken, userData);

        // Initialize automatic token refresh
        initTokenRefresh();

        console.log('✅ SSO authentication configured successfully');
      } else {
        console.error('❌ Failed to generate SSO token');
      }
      // ========== END ENHANCED SSO INTEGRATION ==========
  ...
  }, [isLoggedIn, userData, error, navigate, formData.password]);
      ...
      <div className="w-full max-w-md shadow-2xl border border-indigo-100 rounded-2xl bg-white overflow-hidden">
        ...
        <div className="px-8 pt-6 pb-2 text-center">
          <div className="flex justify-center items-center">
            <h1 className="text-2xl font-bold text-[#7BAA3E]">
              {typedText}
              {isTyping && (
                <span className="ml-1 inline-block w-[2px] h-7 bg-[#7BAA3E] animate-pulse"></span>
              )}
            </h1>
