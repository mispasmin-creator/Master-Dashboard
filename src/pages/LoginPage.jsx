...
useEffect(() => {
  if (isLoggedIn && userData) {
    ...
    // ========== ADD THESE LINES FOR SSO ==========
    // Create SSO token (simple version)
    ...
    // Store SSO token in multiple places
    localStorage.setItem('sso_token', ssoToken);
    sessionStorage.setItem('sso_token', ssoToken);
    
    // Also store in a special format for apps
    const appCredentials = {
      username: userData.username,
      password: userData.password || 'sso_authenticated', // Don't store actual password
      timestamp: Date.now(),
      token: ssoToken
    };
